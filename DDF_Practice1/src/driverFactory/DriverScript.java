...
public class DriverScript extends AppUtile {
	...
	String outputpath="./TestOutput/OrangehrmResults.xlsx";
	@Test
	public void startTest() throws Throwable
	{
		...
		int rc=xl.rowCount("OrangeHRM");
		Reporter.log("Number of Rows are:::: "+rc,true);
		for (int i = 1; i <=rc ; i++) {
			String USERNAME=xl.getCellData("OrangeHRM", i, 0);
			String PASSWORD=xl.getCellData("OrangeHRM", i, 1);
			//Call Login method
			boolean res=FunctionLibrary.Login(USERNAME, PASSWORD);
			if (res) {
				//if res is true write as Login succesLogins into Result Cell
				xl.setCellData("OrangeHRM", i, 2, "Login success",outputpath);
				xl.setCellData("OrangeHRM", i, 3, "PASS",outputpath);
			}
			else
			{
				//if res is false write as Login Fail into Result Cell
				xl.setCellData("OrangeHRM", i, 2, "Login Failure",outputpath);
				xl.setCellData("OrangeHRM", i, 3, "FAIL",outputpath);
